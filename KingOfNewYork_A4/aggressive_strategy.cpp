...
void Aggressive::resolve_dice(Player *p, std::vector<Player*> pls) {
    ...
    while (!mapping.empty()) {
        ...
        if (itr->first == "ATTACK")
            aggressive_d_attack(itr->second, p, pls);
        else if (itr->first == "DESTRUCTION")
            aggressive_d_destruction(itr->second, p, pls);
        else
...
void Aggressive::aggressive_d_attack(int x, Player *p, std::vector<Player *> pls) {
    if (p->m_in_manhattan) {
...
void Aggressive::choose_to_move(Player* p) {
    if (p->get_lifepoints() < 4) {  // an Aggressive player will not leave Manhattan when attacked unless his Life_Points are too low
        p->attacked = true;
        flee(p, p->get_location());
        p->attacked = false;
    }
    else
        std::cout << "\n- \"Never! I am an Aggressive_Player, and Manhattan is mine!\"" << std::endl;
}


void Aggressive::move(Player* p) {
...
bool Aggressive::flee(Player* p, std::string current_loc) {
...
void Aggressive::aggressive_d_destruction(int x, Player *p, std::vector<Player *> pls) {
...
void Aggressive::buy_cards(Player* p, std::vector<Card>& v, std::stack<Card>& s) {
    
}



void Aggressive::find_aggressive_dice(Player* p) {
    int turns = 1;
    while (turns <= 2) {
...
void Aggressive::starting_position(Player* p) {
